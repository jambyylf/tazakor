...
// ТазаКөр: интерфейс тілін браузердің тілінен ажырату.
//
// Chrome-да қазақ тілі МҮЛДЕМ жоқ — браузердің Locales қалтасында kk.pak
// файлы жоқ, сондықтан chrome.i18n ешқашан «kk» қайтармайды, ал _locales/kk
// қалтасы үнсіз еленбейді. Қазақша интерфейсті көрсетудің жалғыз жолы —
// getMessage әдісін өзіміз алмастырып, хабарламаларды таңдалған тілдің
// файлынан оқу.
//
// Тіл ЕКІ көзден оқылады, екеуі де СИНХРОНДЫ:
//   1. ?lang= URL параметрі — use_dynamic_url арқылы жүктелетін беттер үшін
//      (picker, unpicker, zapper, strictblock). Олар бөлек origin-де
//      ашылатындықтан localStorage оларға жетпейді.
//   2. localStorage — қалған беттер үшін.
//
// chrome.storage жарамайды: ол async, ал төмендегі i18n.render() модуль
// деңгейінде синхронды шақырылады. Кесте сол сәтте дайын болмаса, интерфейс
// ЕШҚАНДАЙ ҚАТЕСІЗ бос қалады.
//
// Хабарламалар желіден емес, кеңейтім пакетінің ішінен оқылады.
...
if ( uiLanguage !== '' ) {
    const runtime =
        ...
        self.browser instanceof Element === false
            ...
            : self.chrome.runtime;
    const readMessages = lang => {
        try {
            const xhr = new XMLHttpRequest();
            xhr.open('GET', runtime.getURL(`/_locales/${lang}/messages.json`), false);
            xhr.send();
    ...
    const table = new Map();
    // Әдепкі тіл алдымен оқылады, таңдалған тіл оның үстіне жазылады. Сонда
    // аудармасы жоқ кілт бос қалмай, ағылшыншамен толтырылады.
    for ( const lang of [ 'en', uiLanguage ] ) {
        const json = readMessages(lang);
        if ( json === null ) { continue; }
        for ( const key of Object.keys(json) ) {
            const entry = json[key];
            if ( entry instanceof Object === false ) { continue; }
            if ( typeof entry.message !== 'string' ) { continue; }
            table.set(key, entry.message);
        }
    }
    if ( table.size !== 0 ) {
        const nativeGetMessage = i18n.getMessage.bind(i18n);
        const getMessage = (key, ...args) => {
            if ( typeof key !== 'string' ) { return ''; }
            // Мәтіннің бағыты (rtl/ltr) осы мәнмен шешіледі, сондықтан оны
            // таңдалған тілден синтездейміз.
            if ( key === '@@ui_locale' ) { return uiLanguage; }
            // Қалған @@-кілттер браузердің өз меншігі. Әсіресе
            // @@extension_id: ол бұзылса, элемент таңдағыш ҮНСІЗ ашылмай
            // қалады (tool-overlay.js оны postMessage targetOrigin ретінде
            // қолданады).
            if ( key.startsWith('@@') ) { return nativeGetMessage(key, ...args); }
            const text = table.get(key);
            // Міндетті түрде бос жол қайтарылады, undefined емес: шақыратын
            // жерлердің бірқатары нәтижеге бірден .replace() қолданады.
            return typeof text === 'string' ? text : '';
        };
        try {
            Object.defineProperty(i18n, 'getMessage', {
                value: getMessage, writable: true, configurable: true,
            });
        } catch {
            i18n.getMessage = getMessage;
        }
    }
}
